...
export default function VideoFeed({
  ...
  const drawDetections = useCallback((detections: Detection[], inferenceTime: number, proximityAlerts?: ProximityAlert[]) => {
    ...
    detections.forEach((det) => {
      ...
      const color = CLASS_COLORS[det.className] || CLASS_COLORS.default;
  ...
  return (
    <div className={clsx('bg-gray-900 rounded-xl overflow-hidden', className)}>
      ...
      {showStats && cameraStatus === 'active' && (
        <div className="px-4 py-3 bg-gray-800 grid grid-cols-3 gap-4">
          <div className="text-center">
            <p className="text-2xl font-bold text-white">
              {detection?.pigletCount ?? 0}
            </p>
            <p className="text-xs text-gray-400">Piglets</p>
          </div>
          <div className="text-center">
          ...
          <div className="flex flex-wrap gap-2">
            {detection.detections.slice(0, 8).map((det, idx) => (
              <span
                ...
                style={{ 
                  backgroundColor: `${CLASS_COLORS[det.className] || CLASS_COLORS.default}30`,
                  color: CLASS_COLORS[det.className] || CLASS_COLORS.default
                }}
              >
